...
| <a name="input_daos_ca_secret_id"></a> [daos\_ca\_secret\_id](#input\_daos\_ca\_secret\_id) | ID of Secret Manager secret used to store daosCA.tar.gz file generated on first DAOS server instance | `string` | n/a | yes |
...
| <a name="input_ssh_user"></a> [ssh\_user](#input\_ssh\_user) | Sets user used to SSH, so that it could sets proper permissions on files | `string` | `null` | no |
